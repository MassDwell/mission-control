...
# FIX 2026-03-29: removed set -euo pipefail (macOS bash compat)
# FIX 2026-03-29: removed dead MC file checks (mission-control dismantled)
...
# CHECK 3: Key Log Freshness (replaces dead MC file checks)
echo "CHECK 3: Key Log Freshness"
now=$(date +%s)
for logfile in paperclip.log paperclip-notifier.log telegram-adapter.log; do
    filepath="$HOME/.openclaw/workspace/data/logs/$logfile"
    if [ -f "$filepath" ]; then
        mtime=$(stat -f%m "$filepath" 2>/dev/null || echo "$now")
        age=$((now - mtime))
        age_min=$((age / 60))
        if [ $age_min -lt 60 ]; then
            echo "  ✅ $logfile: active ($age_min min ago)"
        else
            echo "  ⚠️  $logfile: stale ($age_min min ago)"
        fi
    else
...
# CHECK 4: Cron Job Count Sanity
echo "CHECK 4: Cron Job Count Sanity"
if [ -f "$CRON_JOBS" ]; then
    job_count=$(python3 -c "import json; print(len(json.load(open('$CRON_JOBS')).get('jobs',[])))" 2>/dev/null || echo "unknown")
    echo "  ✅ Cron jobs in scheduler: $job_count"
else
    echo "  ⚠️  Cannot read cron jobs file"
fi
echo ""
